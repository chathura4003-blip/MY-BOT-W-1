  ...
  for (let i = 0; i < chain.length; i++) {
    ...
    try {
      ...
      if (!isAudio && FFMPEG_PATH) {
        ...
        if (sizeMB > 90 || ext !== ".mp4") {
          try {
            const msgTxt =
              sizeMB > 50
                ? `⚙️ Compressing ${sizeMB.toFixed(1)}MB video for WhatsApp...`
                : `⚙️ Optimizing format for WhatsApp...`;
            await sock.sendMessage(from, { edit: ph.key, text: msgTxt });
          } catch { }
          downloadedFile = await _compress(downloadedFile, sizeMB > 50);
        } else {
          try {
            await sock.sendMessage(from, {
              edit: ph.key,
