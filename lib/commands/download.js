  ...
  async execute(sock, msg, from, args) {
    ...
    if (!url && keyword) {
      ...
      try {
        ...
        let listMsg = `🌸 ━━━ ❨ ${site.name.toUpperCase()} SEARCH ❩ ━━━ 🌸\n\n`;
        listMsg += `🎐 User: @${sender.split('@')[0]}\n`;
        listMsg += `🎐 Query: "${truncate(keyword, 30)}"\n`;
        listMsg += `⸻⸻⸻⸻⸻⸻⸻\n\n`;
        listMsg += `『 ☁️ SEARCH RESULTS 』\n\n`;
        results.forEach((v, i) => {
          listMsg += `> ✿ ${i + 1}. ${truncate(v.title, 40)}\n`;
          listMsg += `> ⏱️ Duration: ${v.duration || "?"}\n\n`;
        });
        listMsg += `『 ☁️ ACTION 』\n\n`;
        listMsg += `> ✿ 👉 Reply 1–${results.length} to select\n\n`;
        listMsg += `ೃ⁀➷ 💮 CHATHU MD 💮 ೃ⁀➷`;
