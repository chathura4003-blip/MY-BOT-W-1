  ...
  async execute(sock, msg, from, args) {
    ...
    if (!url && keyword) {
      ...
      try {
        ...
        let listMsg = theme.header(toFancy(`${site.name} Search`), emoji) + "\n";
        listMsg += theme.field(toFancy("Query"), `"${truncate(keyword, 30)}"`, "🔍") + "\n";
        listMsg += theme.divider + "\n";

        const items = results.map(
          (v) => `${truncate(v.title, 40)} _(${v.duration || "?"})_`,
        );
        listMsg += theme.list(items) + "\n";
        listMsg += theme.divider + "\n";
        listMsg += theme.line(toFancy(`👉 Reply 1–${results.length} to select`)) + "\n";
        listMsg += theme.footer();
