  ...
  async execute(sock, msg, from, args) {
    ...
    switch (cmd) {
      ...
      case "bal": {
        ...
        const reply = 
          theme.header(toFancy("Financial Status"), "💰") + "\n" +
          theme.field(toFancy("Coins"), user.coins.toLocaleString(), "🪙") + "\n" +
          theme.field(toFancy("XP Level"), user.xp || 0, "⭐") + "\n" +
          theme.field(toFancy("Assets"), user.items?.length ? user.items.join(", ") : "None", "🎒") + "\n" +
          theme.footer();
      ...
      case "shop": {
        const { theme } = require("../utils");
        const { toFancy } = require("../premium");
        let shopMsg = theme.header(toFancy("Premium Shop"), "🛒") + "\n";
        SHOP_ITEMS.forEach((item) => {
          shopMsg += theme.line(`*${toFancy(item.name)}*`) + "\n";
          shopMsg += theme.line(`  💰 ${item.price} — ${item.desc}`) + "\n";
          shopMsg += theme.line(`  _Buy: .buy ${item.id}_`) + "\n";
        });
        shopMsg += theme.footer();
