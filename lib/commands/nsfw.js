  ...
  async execute(sock, msg, from, args) {
    ...
    if (cmd === "nsfwtoggle" || cmd === "nsfw") {
      ...
      await msgMgr.send(sock, from, {
        text: theme.header(toFancy("Privacy Update"), "🔞") + "\n" +
              theme.field(toFancy("NSFW Mode"), val.toUpperCase(), "⚠️") + "\n" +
              theme.footer(),
      });
    ...
    try {
      ...
      await sock.sendMessage(from, {
        image: { url },
        caption: theme.header(toFancy(cmd.toUpperCase()), "🔞") + "\n" +
                 theme.line(toFancy("Adult Content Registry")) + "\n" +
                 theme.footer(),
      });
      await sendReact(sock, from, msg, "✅");
    } catch (err) {
