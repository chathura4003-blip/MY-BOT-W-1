      ...
      switch (cmd) {
        ...
        case "lock": {
          ...
          await msgMgr.send(sock, from, { 
            text: theme.header(toFancy("Security Update"), "🔒") + "\n" +
                  theme.line(toFancy("Group status set to ADMIN-ONLY")) + "\n" +
                  theme.footer()
          });
          await sendReact(sock, from, msg, "✅");
          break;
        ...
        case "unlock": {
          ...
          await msgMgr.send(sock, from, { 
            text: theme.header(toFancy("Security Update"), "🔓") + "\n" +
                  theme.line(toFancy("Group status set to PUBLIC")) + "\n" +
                  theme.footer()
          });
          await sendReact(sock, from, msg, "✅");
          break;
        ...
        case "antilink": {
          ...
          const { toFancy } = require("../premium");
          await msgMgr.send(sock, from, {
            text: theme.header(toFancy("Matrix Shield"), "🛡️") + "\n" +
                  theme.field(toFancy("Anti-Link"), val.toUpperCase(), "💠") + "\n" +
                  theme.footer(),
          });
          await sendReact(sock, from, msg, "✅");
          break;
