...
function formatList(results, query, emoji, label, sender) {
  ...
  msg += `🎐 Query: "${truncate(query, 30)}"\n`;
  msg += `⸻⸻⸻⸻⸻⸻⸻\n\n`;
  msg += `『 ☁️ SEARCH RESULTS 』\n\n`;
  results.forEach((v, i) => {
    msg += `> ✿ ${i + 1}. ${truncate(v.title, 40)}\n`;
    msg += `> ⏱️ Duration: ${v.duration || "?"}\n\n`;
  });
  msg += `『 ☁️ ACTION 』\n\n`;
  ...
  async execute(sock, msg, from, args) {
    ...
    try {
      ...
      if (command === "g") {
        ...
        let reply = `🌸 ━━━ ❨ DUCKDUCKGO ❩ ━━━ 🌸\n\n`;
        reply += `🎐 User: @${sender.split('@')[0]}\n`;
        reply += `🎐 Query: "${truncate(q, 40)}"\n`;
        reply += `⸻⸻⸻⸻⸻⸻⸻\n\n`;
        reply += `『 ☁️ SEARCH RESULT 』\n\n`;
        if (data?.AbstractText) {
          reply += `> ✿ Summary\n`;
          reply += `> ⏱️ ${truncate(data.AbstractText, 400)}\n\n`;
        }
        const topics = (data?.RelatedTopics || []).slice(0, 5);
        if (topics.length > 0) {
          reply += `『 ☁️ RELATED TOPICS 』\n\n`;
          topics.forEach((r, i) => {
      ...
      if (command === "wiki") {
        ...
        let reply = `🌸 ━━━ ❨ WIKIPEDIA ❩ ━━━ 🌸\n\n`;
        reply += `🎐 User: @${sender.split('@')[0]}\n`;
        reply += `🎐 Topic: ${truncate(data.title, 60)}\n`;
        reply += `⸻⸻⸻⸻⸻⸻⸻\n\n`;
        reply += `『 ☁️ SUMMARY 』\n\n`;
        reply += `> ✿ ${truncate(data.extract, 600)}\n\n`;
        if (data?.content_urls?.desktop?.page) {
           reply += `> ⏱️ Link: ${data.content_urls.desktop.page}\n\n`;
        }
        reply += `ೃ⁀➷ 💮 CHATHU MD 💮 ೃ⁀➷`;
      ...
      if (command === "reddit") {
        ...
        let reply = `🌸 ━━━ ❨ REDDIT ❩ ━━━ 🌸\n\n`;
        reply += `🎐 User: @${sender.split('@')[0]}\n`;
        reply += `🎐 Subreddit: r/${truncate(q, 30)}\n`;
        reply += `⸻⸻⸻⸻⸻⸻⸻\n\n`;
        reply += `『 ☁️ HOT POSTS 』\n\n`;
        posts.slice(0, 8).forEach((p, i) => {
          reply += `> ✿ ${i + 1}. ${truncate(p.title, 55)}\n`;
          reply += `> ⏱️ 👍 ${p.ups || 0} | 💬 ${p.num_comments || 0}\n\n`;
        });
        reply += `ೃ⁀➷ 💮 CHATHU MD 💮 ೃ⁀➷`;
      ...
      if (command === "pinsearch") {
        ...
        reply += `🎐 Query: "${truncate(q, 40)}"\n`;
        reply += `⸻⸻⸻⸻⸻⸻⸻\n\n`;
        reply += `『 ☁️ PINTEREST SEARCH 』\n\n`;
        reply += `> ✿ Link: ${link}\n`;
        reply += `> ⏱️ Open in browser\n\n`;
        reply += `ೃ⁀➷ 💮 CHATHU MD 💮 ೃ⁀➷`;
