...
async function showQualityMenu(sock, from, meta, sender) {
  ...
  let menuText = `🌸 ━━━ ❨ VIDEO READY ❩ ━━━ 🌸\n\n`;
  menuText += `🎐 User: @${sender.split('@')[0]}\n`;
  menuText += `⸻⸻⸻⸻⸻⸻⸻\n\n`;
  menuText += `『 ☁️ DETAILS 』\n\n`;
  menuText += `> ✿ Title\n`;
  menuText += `> ⏱️ ${truncate(meta.title, 45)}\n\n`;
  menuText += `> ✿ Duration\n`;
  menuText += `> ⏱️ ${meta.duration || "?"}\n\n`;
  menuText += `> ✿ Size\n`;
  menuText += `> ⏱️ ${sizeStr}\n\n`;
  menuText += `『 ☁️ ACTION 』\n\n`;
  menuText += `> ✿ 👉 Reply 1, 2, or 3 to download\n\n`;
  menuText += `> 1. HD Video 🎬\n`;
  menuText += `> 2. SD Video 📱\n`;
  menuText += `> 3. Audio Only 🎵\n\n`;
  menuText += `ೃ⁀➷ 💮 CHATHU MD 💮 ೃ⁀➷`;
  ...
  try {
    ...
    if (/^\d+$/.test(lower)) {
      ...
      if (quotedMsg) {
          ...
          if (quotedText.includes('VIDEO READY') && quotedText.includes('Reply 1, 2, or 3')) {
          ...
          // 2. Search Results Menu Authentication
          if (quotedText.includes('ACTION') && quotedText.includes('Reply 1–')) {
      ...
      // Legacy fallback in case they reply to a message that doesn't strictly match but we know it's a valid selection 
      // (Leaving stanzaId/lastSearch check just in case it breaks older prompts)
      const fallbackCtxId = msg.message?.extendedTextMessage?.contextInfo?.stanzaId;
      const fallbackKey = fallbackCtxId ? `${sender}:${fallbackCtxId}` : null;
      const fallbackEntry = (fallbackKey && searchResults.get(fallbackKey)) || lastSearch.get(sender);
      if (fallbackEntry && idx >= 0 && idx < fallbackEntry.results.length && !quotedMsg) {
          sendReact(sock, from, msg, "🎬");
          const meta =
            (await safeExecute(
              () => getMetadata(fallbackEntry.results[idx].url),
              "GetMetadataFromSearch",
            )) || fallbackEntry.results[idx];
          await showQualityMenu(sock, from, meta, sender);
          return true;
      }
