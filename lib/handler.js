...
async function handleCommand(sock, msg, from, text, disabledModules = [], context = {}) {
  ...
  try {
    ...
    if (!cmdText || msg.key.fromMe) return false;
        ...
        if (aiCmd && typeof aiCmd.generateAIResponse === "function") {
          (async () => {
            try {
              ...
              logger(`[AI-Auto] Processing for ${sender} in ${from} (Session: ${options.sessionId || 'main'})`);
              await presenceUpdate(sock, from, "composing");
              
              const persona = options.aiAutoPersona || appState.getAiAutoPersona() || "friendly";
              const lang = options.aiAutoLang || appState.getAiAutoLang() || "mixed";
              const useVoice = options.aiAutoVoice !== undefined ? options.aiAutoVoice : appState.getAiAutoVoice();
